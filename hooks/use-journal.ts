...
import { getJournalEntries, addJournalEntry, updateJournalEntry, deleteJournalEntry } from "@/lib/database/journal"
...
export function useJournal() {
  ...
  const addEntryMutation = useCallback(async (newEntryData: Omit<JournalEntryInsert, "user_id">) => {
    startTransition(async () => {
      ...
      try {
        ...
        setEntries((prev) => prev.filter((entry) => !entry.id.startsWith("temp-"))) // Rollback optimistic update
      }
    })
  }, [])
